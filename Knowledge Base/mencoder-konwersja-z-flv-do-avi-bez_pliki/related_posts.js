...
function BRPS_render_widget_title() {
  ...
  if (brps_options && brps_options.title)
    ...
    $('<h2>Podobne wpisy:</h2><br />').appendTo('#related_posts');
  }
...
function BRPS_get() {
  ...
  if (blog_id != '' && post_id != '') {
    ...
    $('<i>Ładowanie...</i>').appendTo('#related_posts');
          ...
          if (data.error) {
            ...
            if (data.entry.length > 0) {
              ...
              $.each(data.entry, function(i, entry){
                $('<li><a hr' + 'ef="' + entry.link + src + '" title="Zgodność: ' + entry.score.toString() + '">' + entry.title + '</a></li>').appendTo('#related_posts ul');
                });
              }
            else {
              $('<p>Nie ma podobnych wpisów.</p>').appendTo('#related_posts');
              }
            }
        });
    }
